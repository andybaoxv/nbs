...
% How to set Gamma, the coefficient controlling the regularization term
optionDefault.optGamma = 1;
optionDefault.optGammaIterMax = optionDefault.iter/2;
optionDefault.optGammaIterMin = 0;

% Use default parameters or user specified parameters
...
% Random initialization of matrix H
...
% Random initialization of matrix W, not that it's better to make Y~WH
...
% K is regularization matrix, we should make sure #genes be equal to size
% K is the graph laplacian matrix
% of matrix K
if ( r > length(K) )
...
for i=1:option.iter         
    ...
    W = W.*((Ydf*H' + fgamma*KWmat_W + eps )./(W*(H*H') + fgamma*KWmat_D + eps));
    W = max(W,eps);
